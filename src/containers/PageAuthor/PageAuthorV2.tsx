...
import { PostAuthorType, PostDataType, FakeAuthorType } from "data/types";
...
const FAKE_AUTHOR: FakeAuthorType = DEMO_FAKE_SKELETON_USERS[0];
const FILTERS = [
  { name: "Most Recent" },
...
useEffect(() => {
    if( tabActive === 'Articles' ){
      dispatch(fetchUserPublicBlogs(id, filter)).then((res) => {
      ...
      dispatch(categoryWithTotalBlogs(filterCategory)).then((res) => {
